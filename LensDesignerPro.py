...
"""
LensDesignerPro.py
MICRO LED Lens Designer Pro - Confirmed V10 Geometry
...
class LensDesignerPro:
    def __init__(self, root):
        self.root = root
        self.root.title("MICRO LED Lens Designer Pro - V10")
    ...
    def build_ui(self):
        ...
        tk.Label(
            top,
            text="MICRO LED Lens Designer Pro",
    ...
    def build_param_panel(self, parent):
        ...
        self.button(actions, "Run Simulation", self.run_simulation, bg=GOOD, width=26).pack(fill="x", pady=5)
    ...
    def draw_illuminance(self, vals, waist):
        ...
        z = np.linspace(-0.35, 0.35, 280)
        X, Z = np.meshgrid(x, z)

        rise = 1 / (1 + np.exp(-(X - 0.52) / 0.060))
        fall = 1 / (1 + np.exp((X - 7.0) / 0.230))
        x_profile = rise * fall
        x_profile *= 1.0 + 0.018 * np.cos((X - 3.7) / 3.3 * np.pi)

        # Approximate width control: normalized around 0.30m target
        target_half_width = 0.15
        z_profile = 1 / (1 + np.exp((np.abs(Z) - target_half_width) / 0.018))
        ...
        cf = self.ax_iso.contourf(X, Z, E, levels=np.linspace(0, 1, 24), cmap="turbo")
        self.ax_iso.contour(X, Z, E, levels=[0.1, 0.5, 0.8], colors="white", linewidths=0.8)
        self.ax_iso.axvline(0.5, color="white", linestyle="--", linewidth=1)
        self.ax_iso.axvline(7.0, color="white", linestyle="--", linewidth=1)
        self.ax_iso.axhline(0.15, color="white", linestyle="--", linewidth=0.8)
        self.ax_iso.axhline(-0.15, color="white", linestyle="--", linewidth=0.8)
        self.ax_iso.text(1.2, 0.29, "flat-top target", color="white", fontsize=8)
        ...
        self.ax_line.axvline(0.5, color="white", linestyle="--", linewidth=1)
        self.ax_line.axvline(7.0, color="white", linestyle="--", linewidth=1)
        ...
        self.ax_line.text(0.9, 0.10, f"L={L:g}, La={La:g}, Lb={Lb:g}, Ra={Ra:g}\nwaist={waist:.1f}mm", color=TEXT_FG, fontsize=8)
